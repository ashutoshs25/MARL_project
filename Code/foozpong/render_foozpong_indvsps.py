...
parser.add_argument("checkpoint_path_2",help="second checkpoint")

args = parser.parse_args()


print(args.checkpoint_path)
print(args.checkpoint_path_2)
checkpoint_path = os.path.expanduser(args.checkpoint_path)
checkpoint_path_2 = os.path.expanduser(args.checkpoint_path_2)
params_path = Path(checkpoint_path).parent.parent/"params.pkl"
params_path_2 = Path(checkpoint_path_2).parent.parent/"params.pkl"
...
with open(params_path_2, "rb") as f:
    config_2 = pickle.load(f)
    # num_workers not needed since we are not training
    del config_2['num_workers']
    del config_2['num_gpus']
...
PPOagent_2 = PPOTrainer(env=env_name, config=config_2)
PPOagent_2.restore(checkpoint_path_2)
...
for agent in env.agent_iter():
    ...
    if done:
        ...
        if agent == 'first_0' or agent == 'third_0':
            action, _, _ = PPOagent.get_policy("shared_policy_1").compute_single_action(observation)
        elif agent == 'second_0':
            action, _, _ = PPOagent_2.get_policy("shared_policy_1").compute_single_action(observation)
        else:
            action, _, _ = PPOagent_2.get_policy("shared_policy_3").compute_single_action(observation)
        #action, _, _ = PPOagent.get_policy("shared_policy_1").compute_single_action(observation)
...
print(reward_sum)
frame_list[0].save("out_foozpong_indvsps.gif", save_all=True, append_images=frame_list[1:], duration=3, loop=0)
